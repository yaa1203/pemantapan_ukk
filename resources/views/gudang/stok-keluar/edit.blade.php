<x-app-layout>
    <div class="p-6 max-w-xl space-y-6">
        ...
        <div class="bg-white border border-gray-100 rounded-2xl p-6 space-y-4">
            <form action="{{ route('stok-keluar.update', $stokMasuk) }}" method="POST">
                @csrf @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Gerai</label>
                        <select name="gerai_id" class="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-amber-300">
                            @foreach($gerais as $gerai)
                                <option value="{{ $gerai->id }}" {{ $stokMasuk->gerai_id == $gerai->id ? 'selected' : '' }}>
                                    {{ $gerai->nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('gerai_id') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
